...
with open(training_data_path, 'r') as f:
    text = f.read()
text = text[:10000] # truncate to 10k characters
tokens = enc.encode(text)
B, T = 4, 32
buf = torch.tensor(tokens[: B*T + 1])
# move the buf tensor to device
# need to reassign the buf tensor to the device, as the original tensor is on the CPU
buf = buf.to(device)
x = buf[:-1].view(B, T)
y = buf[1:].view(B, T)
