...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        """ Flash Attention: prefer SDPA; provide FP64 XPU fallback """
        # For FP64 on XPU, try SDPA in float32 to avoid unsupported/expensive FP64 kernels;
        # if SDPA still fails, fall back to chunked manual attention in float32 to reduce memory.
        if x.device.type == "xpu" and x.dtype == torch.float64:
            q32, k32, v32 = q.float(), k.float(), v.float()
            try:
                y32 = F.scaled_dot_product_attention(q32, k32, v32, is_causal=True)
            except Exception:
                inv_sqrt = 1.0 / math.sqrt(k32.size(-1))
                kT = k32.transpose(-2, -1)
                ...
                for qs in range(0, T, chunk):
                    ...
                    q_s = q32[:, :, qs:qe, :]
                    att = (q_s @ kT) * inv_sqrt
                    att = att.masked_fill(self.bias[:, :, qs:qe, :T] == 0, float('-inf'))
                    att = F.softmax(att, dim=-1)
                    y_s = att @ v32
                    ys.append(y_s)
                y32 = torch.cat(ys, dim=2)
            y = y32.to(x.dtype)
        else:
...
class DataLoaderLite:
    def __init__(self, B, T, data_path, model='gpt2', dtype: torch.dtype = torch.long):
        self.B = B
        self.T = T
        ...
        # allow user-specified dtype for initial tensor construction
        self.tokens = torch.tensor(tokens, dtype=self.tensor_dtype)
    ...
    def next_batch(self):
        ...
        # embeddings require long dtype indices
...
# Toggle FP64 on XPU for higher numerical precision
USE_XPU_FP64 = True
...
if torch.cuda.is_available():
    ...
    device = "xpu"
    print_xpu_info()
print(f"using device: {device}{' | XPU FP64' if (device=='xpu' and USE_XPU_FP64) else ''}")
...
if device == "xpu" and USE_XPU_FP64:
    train_loader = DataLoaderLite(B=4, T=1, data_path=training_data_path, dtype=torch.float64)
else:
...
if device == 'xpu' and USE_XPU_FP64:
    # ensure new tensors default to float64 before moving
...
# Optimizer doesn't work for FP64
...
if device == "cuda":
    ...
    if USE_XPU_FP64:
        # Keep FP64 params and use standard PyTorch AdamW (IPEX optimizer requires FP32)
        # Optimize only the model to avoid replacing optimizer with IPEX's FP32-only variant.
        model, optimizer = ipex.optimize(model, optimizer=optimizer, dtype=torch.float64)
    else:
